...
const GAMES = {
  game_coinflip: { label: 'Coinflip', emoji: '🪙', description: 'Kopf oder Zahl – 50/50', titel: 'Coinflip' },
  game_wuerfel: { label: 'Würfelduell', emoji: '🎲', description: 'Würfel 4-6 gewinnt, 1-3 verliert – 50/50', titel: 'Würfelduell' },
  game_karte: { label: 'Kartenziehen', emoji: '🃏', description: 'Rot oder Schwarz – 50/50', titel: 'Kartenziehen' },
};
...
async function handleGameModalSubmit(interaction) {
  ...
  const spielInfo = GAMES[spiel] || { titel: 'Spiel', emoji: '🎮' };
  ...
  let ergebnisText;
  if (spiel === 'game_wuerfel') {
    const wurf = gewonnen ? 4 + Math.floor(Math.random() * 3) : 1 + Math.floor(Math.random() * 3);
    ergebnisText = `Du hast eine **${wurf}** gewürfelt.`;
  } else if (spiel === 'game_karte') {
    ergebnisText = `Die Karte war **${gewonnen ? 'Rot' : 'Schwarz'}**.`;
  } else {
    ergebnisText = `Die Münze zeigte **${gewonnen ? 'Kopf' : 'Zahl'}**.`;
  }
  ...
  await interaction.reply({ embeds: [embed], components: [buildGameSelectRow()], ephemeral: true });
