...
const GAMES = {
  ...
  game_wuerfel: {
    label: 'Würfelduell', emoji: '🎲', titel: 'Würfelduell', type: 'choice',
    description: 'Hoch (4-6) oder Niedrig (1-3) wählen – 50/50',
    choices: [
      { key: 'hoch', label: 'Hoch (4-6)', emoji: '⬆️' },
      { key: 'niedrig', label: 'Niedrig (1-3)', emoji: '⬇️' },
    ],
    frames: ['🎲 Der Würfel rollt...', '🎲 ...er wackelt...', '🎲 ...kommt zur Ruhe...'],
    resolve: () => {
      const wurf = 1 + Math.floor(Math.random() * 6);
      const outcomeKey = wurf >= 4 ? 'hoch' : 'niedrig';
      return { outcomeKey, displayText: `${wurf} (${outcomeKey === 'hoch' ? 'Hoch' : 'Niedrig'})` };
    },
  },
  game_karte: {
  ...
  game_crash: {
    label: 'Crash', emoji: '🚀', titel: 'Crash', type: 'crash',
    description: 'Multiplikator steigt live – steig rechtzeitig aus!',
  },
  game_tresor: {
...
// aktive Crash-Spiele: token -> { userId, einsatz, multiplier, crashPoint, interval, interaction, ended }
...
function buildCrashEmbed(multiplier) {
  return new EmbedBuilder()
    .setTitle('🚀 Crash')
    .setDescription(`Aktueller Multiplikator: **${multiplier.toFixed(2)}x**\n\nDrück auf "Aussteigen", bevor es crasht!`)
    .setColor(0x9b59b6);
}

function buildCrashRow(token) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`crash_cashout::${token}`).setLabel('🛑 Aussteigen').setStyle(ButtonStyle.Danger)
  );
}
...
client.on('interactionCreate', async (interaction) => {
  try {
    if (interaction.isChatInputCommand()) {
      ...
      } else if (interaction.customId.startsWith('crash_cashout::')) {
        await handleCrashCashoutButton(interaction);
...
async function handleGameModalSubmit(interaction) {
  ...
  // ---- CRASH: startet direkt das Live-Spiel mit Aussteigen-Knopf ----
  if (spielInfo.type === 'crash') {
    const token = generateToken();
    await interaction.reply({ embeds: [buildCrashEmbed(1.0)], components: [buildCrashRow(token)], ephemeral: true });

    const r = Math.random();
    const crashPoint = Math.min(20, 1 / (1 - r * 0.995)); // "provably fair": P(crash > 2x) = 50%

    const game = { userId: interaction.user.id, einsatz, multiplier: 1.0, crashPoint, interaction, ended: false, interval: null };
    crashGames.set(token, game);

    game.interval = setInterval(async () => {
      if (game.ended) { clearInterval(game.interval); return; }
      game.multiplier = Math.round(game.multiplier * 1.12 * 100) / 100;

      if (game.multiplier >= game.crashPoint) {
        game.ended = true;
        clearInterval(game.interval);
        crashGames.delete(token);
        const neuerKontostand = getGuthaben(game.userId) - game.einsatz;
        setGuthaben(game.userId, neuerKontostand);
        const finalEmbed = new EmbedBuilder()
          .setTitle('🚀 Crash!')
          .setDescription(`💥 Bei **${game.crashPoint.toFixed(2)}x** gecrasht! Du hast **${formatGuthaben(game.einsatz)}** WindSMP-Coins verloren.`)
          ...
          .setColor(0xe74c3c);
        try {
          await game.interaction.editReply({ embeds: [finalEmbed], components: [buildGameSelectRow()] });
        } catch (e) { /* Nachricht evtl. schon weg */ }
        setTimeout(async () => { try { await game.interaction.deleteReply(); } catch (e) {} }, 60_000);
        return;
      }

      try {
        await game.interaction.editReply({ embeds: [buildCrashEmbed(game.multiplier)], components: [buildCrashRow(token)] });
      } catch (e) {
        game.ended = true;
        clearInterval(game.interval);
        crashGames.delete(token);
  ...
  if (spielInfo.type === 'tresor') {
    ...
    const row = new ActionRowBuilder().addComponents(
      outcomes.map((o, i) =>
        new ButtonBuilder()
          .setCustomId(`tresor_pick::${einsatz}::${o}`)
  ...
  // ---- Standardspiele mit 2 Auswahl-Buttons (Coinflip, Würfelduell, Kartenziehen) ----
  const embed = new EmbedBuilder()
...
async function handleCrashCashoutButton(interaction) {
  const token = interaction.customId.replace('crash_cashout::', '');
  const game = crashGames.get(token);

  if (!game || game.ended) {
    return interaction.reply({ content: 'Dieses Spiel ist bereits beendet.', ephemeral: true });
  }
  ...
  const gewinn = Math.round(game.einsatz * (game.multiplier - 1));
  const neuerKontostand = getGuthaben(game.userId) + gewinn;
  setGuthaben(game.userId, neuerKontostand);

  const embed = new EmbedBuilder()
    .setTitle('🚀 Ausgestiegen!')
    .setDescription(`Du bist bei **${game.multiplier.toFixed(2)}x** ausgestiegen und hast **${formatGuthaben(gewinn)}** WindSMP-Coins gewonnen!`)
    .addFields({ name: 'Neuer Kontostand', value: formatGuthaben(neuerKontostand) })
    .setFooter({ text: 'Wähle unten direkt weiter, wenn du willst. Diese Nachricht löscht sich in 1 Minute.' })
    .setColor(0x2ecc71);

  await interaction.update({ embeds: [embed], components: [buildGameSelectRow()] });
...
async function handleTresorPick(interaction) {
  const [, einsatzStr, outcome] = interaction.customId.split('::');
