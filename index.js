...
async function handleSlashCommand(interaction) {
  ...
  if (commandName === 'livecrash-start') {
    if (liveCrash.running) {
      return interaction.reply({ content: '⚠️ Das Live-Crash-Event läuft bereits.', ephemeral: true });
    }
    await interaction.reply({ content: '🚀 Live-Crash-Event wird gestartet...', ephemeral: true });
    startLiveCrash(interaction.channel);
  }

  if (commandName === 'livecrash-stop') {
    if (!liveCrash.running) {
      return interaction.reply({ content: 'Das Live-Crash-Event läuft gerade nicht.', ephemeral: true });
    }
    stopLiveCrash();
...
function startLiveCrash(channel) {
  liveCrash.running = true;
  liveCrash.channel = channel;
  runLiveCrashWaitingPhase();
}

function stopLiveCrash() {
  liveCrash.running = false;
  clearInterval(liveCrash.countdownInterval);
  clearInterval(liveCrash.runInterval);
  liveCrash.phase = 'idle';
}
...
function buildLiveCrashWaitingEmbed(restSekunden) {
  const teilnehmer = Object.values(liveCrash.participants);
...
function buildLiveCrashRunningEmbed() {
  const aktive = Object.values(liveCrash.participants).filter(p => !p.cashedOut);
  const ausgestiegen = Object.values(liveCrash.participants).filter(p => p.cashedOut);
  ...
  return new EmbedBuilder()
    .setTitle('🚀 LIVE CRASH')
    .setDescription(`**Multiplikator: ${liveCrash.multiplier.toFixed(2)}x**\n\n**Noch dabei:**\n${aktiveListe}\n\n**Ausgestiegen:**\n${ausgestiegenListe}`)
    .setColor(0x9b59b6);
}

async function runLiveCrashWaitingPhase() {
  if (!liveCrash.running) return;

  liveCrash.phase = 'waiting';
  liveCrash.participants = {};
  liveCrash.countdownEnd = Date.now() + WARTEZEIT_SEK * 1000;

  liveCrash.message = await liveCrash.channel.send({
    embeds: [buildLiveCrashWaitingEmbed(WARTEZEIT_SEK)],
    components: [buildLiveCrashJoinRow()],
  });

  liveCrash.countdownInterval = setInterval(async () => {
    if (!liveCrash.running) { clearInterval(liveCrash.countdownInterval); return; }
    const restSekunden = Math.ceil((liveCrash.countdownEnd - Date.now()) / 1000);

    if (restSekunden <= 0) {
      clearInterval(liveCrash.countdownInterval);
      await startLiveCrashRunningPhase();
      return;
    }
    try {
      await liveCrash.message.edit({ embeds: [buildLiveCrashWaitingEmbed(restSekunden)] });
    } catch (e) { /* Nachricht evtl. gelöscht */ }
  }, 1000);
}

async function startLiveCrashRunningPhase() {
  if (!liveCrash.running) return;

  liveCrash.phase = 'running';
  const r = Math.random();
  liveCrash.crashPoint = Math.min(20, 1 / (1 - r * 0.995)); // "provably fair": P(crash > 2x) = 50%
  liveCrash.multiplier = 1.0;

  try {
    await liveCrash.message.edit({ embeds: [buildLiveCrashRunningEmbed()], components: [buildLiveCrashCashoutRow()] });
  } catch (e) {}

  liveCrash.runInterval = setInterval(async () => {
    if (!liveCrash.running) { clearInterval(liveCrash.runInterval); return; }
    liveCrash.multiplier = Math.round(liveCrash.multiplier * 1.04 * 100) / 100;

    if (liveCrash.multiplier >= liveCrash.crashPoint) {
      clearInterval(liveCrash.runInterval);
      await endLiveCrashRound();
      return;
    }
    try {
      await liveCrash.message.edit({ embeds: [buildLiveCrashRunningEmbed()], components: [buildLiveCrashCashoutRow()] });
...
async function endLiveCrashRound() {
  liveCrash.phase = 'crashed';

  const verloren = Object.values(liveCrash.participants).filter(p => !p.cashedOut);
  const gewonnen = Object.values(liveCrash.participants).filter(p => p.cashedOut);
  ...
  const embed = new EmbedBuilder()
    .setTitle('💥 CRASH!')
    .setDescription(`Bei **${liveCrash.crashPoint.toFixed(2)}x** gecrasht!\n\n**Gewonnen:**\n${gewonnenListe}\n\n**Verloren:**\n${verlorenListe}\n\nNächste Runde startet in Kürze...`)
    .setColor(0xe74c3c);

  try {
    await liveCrash.message.edit({ embeds: [embed], components: [] });
  } catch (e) {}

  if (liveCrash.running) {
    setTimeout(runLiveCrashWaitingPhase, RUNDEN_PAUSE_MS);
  }
}

async function handleLiveCrashJoin(interaction) {
  if (liveCrash.phase !== 'waiting') {
    return interaction.reply({ content: 'Die Runde läuft schon oder ist vorbei – warte auf die nächste Runde.', ephemeral: true });
  }
  if (liveCrash.participants[interaction.user.id]) {
...
async function handleLiveCrashBetModalSubmit(interaction) {
  ...
  liveCrash.participants[interaction.user.id] = {
  ...
  try {
    const restSekunden = Math.max(0, Math.ceil((liveCrash.countdownEnd - Date.now()) / 1000));
    await liveCrash.message.edit({ embeds: [buildLiveCrashWaitingEmbed(restSekunden)] });
  } catch (e) {}
}

async function handleLiveCrashCashout(interaction) {
  const teilnehmer = liveCrash.participants[interaction.user.id];

  if (liveCrash.phase !== 'running' || !teilnehmer) {
  ...
  teilnehmer.cashoutMultiplier = liveCrash.multiplier;
  teilnehmer.gewinn = Math.round(teilnehmer.einsatz * liveCrash.multiplier);
  setGuthaben(interaction.user.id, getGuthaben(interaction.user.id) + teilnehmer.gewinn);

  await interaction.reply({
    content: `🛑 Du bist bei **${liveCrash.multiplier.toFixed(2)}x** ausgestiegen und hast **${formatGuthaben(teilnehmer.gewinn)}** WindSMP-Coins erhalten!`,
    ephemeral: true,
  });
}
